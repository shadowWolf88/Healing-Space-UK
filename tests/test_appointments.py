...
def test_analytics_includes_appointments(client, tmp_db, test_patient, test_clinician):
    ...
    cur.execute(
        ...
        "VALUES (?,?,?,?,?)",
        (test_clinician['username'], test_patient['username'], appt_date, 'Follow-up', 'pending')
    )
    
    conn.commit()
    conn.close()
    
    # Authenticate as clinician and request with clinician_username
    ...
    resp = client.get(f"/api/analytics/patient/{test_patient['username']}?clinician_username={test_clinician['username']}")
...
def test_attendance_endpoint_updates_db_and_notifications(client, tmp_db, test_patient, test_clinician):
    ...
    cur.execute(
        ...
        "VALUES (?,?,?,?)",
        (test_clinician['username'], test_patient['username'], appt_date, 'Check-in')
    )
    ...
    resp = client.post(f"/api/appointments/{appt_id}/attendance", json={
        'clinician_username': test_clinician['username'],
        'status': 'attended'
    })
